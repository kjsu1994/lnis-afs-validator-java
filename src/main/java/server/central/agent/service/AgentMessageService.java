  ...
  public void handle(Envelope envelope) throws Exception {
    ...
    switch (envelope.type()) {
      ...
      case STATUS -> {
        ...
        if (progress.type() == null) {
          events.publish(
              ...
              envelope.role(),
              envelope.sessionId(),
              java.util.Map.of("message", "Agent STATUS event type is missing"));
          return;
        }
        events.publish(
            progress.type(), envelope.agentId(), envelope.role(), envelope.sessionId(), progress);
      }
      case INPUT_CHUNK -> {
        // rawSerial은 장치 진단용이며 시험 입력에는 Agent가 변환한 canonical GRAW만 누적한다.
        byte[] canonical =
            Base64.getDecoder().decode(envelope.payload().path("canonicalBase64").asText());
      ...
      case ROLE_RESULT -> {
        ...
        events.publish(
            EventType.RESULT, envelope.agentId(), envelope.role(), envelope.sessionId(), result);
  ...
  private void handleHello(Envelope envelope) throws Exception {
    ...
    agents.save(
        new AgentEntity(
            ...
            null));
    events.publish(EventType.AGENT_STATUS, envelope.agentId(), envelope.role(), null, "READY");
  }
