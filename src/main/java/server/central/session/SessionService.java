...
public class SessionService {
  ...
  public TestSessionEntity create(CreateSessionRequest request) {
    ...
    try {
      ...
      events.publish(
          server.shared.model.AgentProtocol.EventType.SESSION_STATUS, null, null, id, session);
      return session;
    } catch (Exception e) {
  ...
  private TestSessionEntity terminateInternal(UUID id, SessionState terminalState, String reason) {
    try {
      ...
      events.publish(
          server.shared.model.AgentProtocol.EventType.SESSION_STATUS, null, null, id, terminated);
      return terminated;
    } finally {
  ...
  public synchronized void onResult(UUID id) {
    ...
    release(id);
    events.publish(
        server.shared.model.AgentProtocol.EventType.SESSION_STATUS, null, null, id, completed);
  }
