package server.central.artifact.controller;

import java.util.Locale;
import java.util.UUID;
import org.springframework.http.*;
import org.springframework.web.bind.annotation.*;
import server.central.artifact.service.ArtifactService;
import server.central.artifact.service.CombinedArtifactService;
...
public class ArtifactController {
  ...
  public ArtifactController(ArtifactService artifacts, CombinedArtifactService combinedArtifacts) {
  ...
  public ResponseEntity<byte[]> downloadCombined(
    ...
    MediaType mediaType =
        switch (fileName) {
          ...
          default -> throw new IllegalArgumentException("지원하지 않는 통합 산출물입니다: " + fileName);
        };
    return ResponseEntity.ok()
        .contentType(mediaType)
        .header(
            HttpHeaders.CONTENT_DISPOSITION,
            ContentDisposition.attachment().filename(sessionId + "-" + fileName).build().toString())
        .body(combinedArtifacts.create(sessionId, fileName));
  }

  @GetMapping("/{sessionId}/artifacts/{role}/{fileName}")
  public ResponseEntity<byte[]> download(
      @PathVariable UUID sessionId, @PathVariable String role, @PathVariable String fileName) {
    AgentRole agentRole =
