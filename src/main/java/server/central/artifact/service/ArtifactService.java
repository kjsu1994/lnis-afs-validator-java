...
import server.protocol.model.LnisModels.RoleResult;

@Service
/**
 * 역할별 시험 결과를 JSON 또는 UTF-8 BOM CSV 산출물로 직렬화한다.
 *
 * <p>파일을 서버 디스크에 미리 만들지 않고 HTTP 요청 시 H2 결과에서 즉시 생성한다. 허용된 세 파일 이름만 switch로 처리해 사용자가 임의의 서버 파일 경로를
 * 지정할 수 없게 한다.
 */
public class ArtifactService {
  ...
  public byte[] create(UUID id, AgentRole role, String fileName) {
    RoleResult result =
        sessions
            .result(id, role)
            .orElseThrow(() -> new IllegalArgumentException("Result not found for " + role));
    try {
  ...
  private static String summary(RoleResult result) {
    ...
    for (var metric : result.metrics()) {
      csv.append(result.role())
          .append(',')
          .append(metric.category())
          .append(',')
          .append(cell(metric.name()))
          .append(',')
          .append(cell(metric.description()))
          .append(',')
          .append(metric.value() == null ? "" : metric.value())
          .append(',')
          .append(cell(metric.unit()))
          .append(',')
  ...
  private static String timeseries(RoleResult result) {
    ...
    for (var sample : result.samples()) {
      csv.append(result.role())
          ...
          .append(',')
          .append(sample.cpuPercent())
          .append(',')
          .append(sample.workingSetBytes())
          .append("\r\n");
    }
    return csv.toString();
  }
