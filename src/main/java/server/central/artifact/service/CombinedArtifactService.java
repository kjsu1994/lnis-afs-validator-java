package server.central.artifact.service;
...
import server.central.frameevidence.dto.FrameEvidenceDetail;
import server.central.frameevidence.dto.FrameEvidenceSummary;
...
public class CombinedArtifactService {
  ...
  public CombinedArtifactService(
      SessionRepository sessions, FrameEvidenceService frameEvidence, ObjectMapper json) {
  ...
  public byte[] create(UUID sessionId, String fileName) {
    ...
    CombinedReport report =
        new CombinedReport(
            1, sessionId, Instant.now(), sender, receiver, frameEvidence.details(sessionId));
    try {
      return switch (fileName) {
        case "lnis-report.json" -> json.writerWithDefaultPrettyPrinter().writeValueAsBytes(report);
        case "lnis-report.xlsx" -> excel(report);
        default -> throw new IllegalArgumentException("Unknown combined artifact: " + fileName);
      };
  ...
  public static class CombinedReport {
    ...
    public CombinedReport(
        ...
        RoleResult receiverResult,
        List<FrameEvidenceDetail> frameEvidence) {
      frameEvidence = frameEvidence == null ? List.of() : List.copyOf(frameEvidence);
  ...
  private byte[] excel(CombinedReport report) throws java.io.IOException {
    try (XSSFWorkbook book = new XSSFWorkbook();
      ...
      book.getProperties().getCoreProperties().setTitle("LNIS 통합 시험 결과 " + report.sessionId());
  ...
  private int flatten(Sheet sheet, int row, Styles styles, String role, String path, Object value)
    ...
    if (value instanceof Map<?, ?> map) {
      for (Map.Entry<?, ?> entry : map.entrySet()) {
        String next = path.isEmpty() ? entry.getKey().toString() : path + "." + entry.getKey();
  ...
  private static void frameSheet(Workbook book, List<FrameEvidenceDetail> details, Styles styles) {
    ...
    finish(
        sheet, row - 1, 18, 10, 13, 11, 11, 11, 11, 16, 16, 16, 12, 36, 13, 13, 13, 28, 28, 28, 28,
        54);
  }

  private static List<RoleResult> results(CombinedReport report) {
    return java.util.stream.Stream.of(report.senderResult(), report.receiverResult())
