...
import server.central.frameevidence.dto.*;
...
public class FrameEvidenceService {
  ...
  public FrameEvidenceService(FrameEvidenceRepository repository, ObjectMapper json) {
  ...
  public byte[] jsonArtifact(UUID sessionId) {
    try {
      return json.writerWithDefaultPrettyPrinter().writeValueAsBytes(details(sessionId));
  ...
  public List<FrameEvidenceDetail> details(UUID sessionId) {
    return summaries(sessionId).stream().map(item -> detail(sessionId, item.frameIndex())).toList();
  }
    ...
    StringBuilder csv =
        new StringBuilder(
            "FrameIndex,DecoderCompleted,FullyDecoded,SB2CrcValid,SB3CrcValid,SB4CrcValid,"
                ...
                + "TransmittedToReceivedBits,ReferenceToReencodedBits,ReferenceSha256,TransmittedSha256,ReceivedSha256,ReencodedSha256,Interpretation\r\n");
    for (FrameEvidenceSummary item : summaries(sessionId)) {
      csv.append(item.frameIndex())
          .append(',')
          .append(item.decoderCompleted())
          .append(',')
          .append(item.decodeSucceeded())
          .append(',')
          .append(item.sb2CrcValid())
          .append(',')
          .append(item.sb3CrcValid())
          .append(',')
          .append(item.sb4CrcValid())
          .append(',')
          .append(item.sb2DecisionChanges())
          .append(',')
          .append(item.sb3DecisionChanges())
          .append(',')
          .append(item.sb4DecisionChanges())
          .append(',')
          .append(item.usedForGrawReassembly())
          .append(',')
          .append(cell(item.failureReason()))
          .append(',')
          .append(number(item.referenceToTransmittedDifferences()))
          .append(',')
          .append(number(item.transmittedToReceivedDifferences()))
          .append(',')
          .append(number(item.referenceToReencodedDifferences()))
          .append(',')
          .append(cell(item.referenceSha256()))
          .append(',')
          .append(cell(item.transmittedSha256()))
          .append(',')
          .append(cell(item.receivedSha256()))
          .append(',')
          .append(cell(item.reencodedSha256()))
          .append(',')
          .append(cell(item.interpretation()))
          .append("\r\n");
    }
  ...
  private Map<Integer, EnumMap<AgentRole, FrameEvidenceMessage>> group(UUID sessionId) {
    ...
    for (FrameEvidenceEntity entity : repository.findAll(sessionId)) {
      grouped
          .computeIfAbsent(entity.frameIndex(), ignored -> new EnumMap<>(AgentRole.class))
  ...
  private static FrameEvidenceSummary summary(
      int frameIndex, FrameEvidenceMessage sender, FrameEvidenceMessage receiver) {
    byte[] reference = bytes(sender, FrameEvidenceMessage::referenceFrame);
    byte[] transmitted = bytes(sender, FrameEvidenceMessage::transmittedFrame);
    byte[] received = bytes(receiver, FrameEvidenceMessage::receivedFrame);
    byte[] reencoded = bytes(receiver, FrameEvidenceMessage::reencodedFrame);
    ...
    return new FrameEvidenceSummary(
        ...
        interpretation(injected, transport, recovered, sender, receiver));
  }
  ...
  private static String interpretation(
    ...
    if (!receiver.decoderCompleted()) {
      return "AFS Decoder가 처리를 완료하지 못했습니다. 원인: " + defaultReason(receiver.failureReason());
    }
    if (!receiver.decodeSucceeded()) {
      String use =
          receiver.usedForGrawReassembly()
  ...
  private static String defaultReason(String value) {
    return value == null || value.isBlank() ? "상세 원인 없음" : value;
  }

  /** 동기 패턴 68비트 안의 의도적 손상 때문에 복호화에서 제외된 Test D 프레임인지 판별한다. */
  private static boolean intentionalSyncDamage(
      FrameEvidenceMessage sender, FrameEvidenceMessage receiver) {
    return sender != null
        && receiver != null
  ...
  private static byte[] bytes(
      FrameEvidenceMessage value, Function<FrameEvidenceMessage, byte[]> getter) {
    return value == null ? null : getter.apply(value);
  }

  private static String hash(byte[] value) {
    return value == null ? null : Hashing.hex(Hashing.sha256Digest().digest(value));
  }
