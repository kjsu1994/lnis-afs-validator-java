 ...
 *
 * <p>모든 packet은 ASCII magic, protocol version, packet 종류, .NET 호환 UUID, sequence, GNSS 시간, payload와
 * CRC32 순서로 구성된다. 숫자는 network byte order(big-endian)를 사용한다. payload 상한을 1200 byte로 제한해 일반적인 MTU
 * 환경에서 IP fragmentation 가능성을 낮춘다.
 */
public final class AfsPacketCodec {
  ...
  public static class Packet {
    ...
    public Packet(
      ...
      this.sentUtcTicks = sentUtcTicks;
  ...
  public static byte[] encode(Packet packet) {
    if (packet.payload.length > MAXIMUM_PAYLOAD_LENGTH)
      throw new IllegalArgumentException("AFS UDP payload exceeds 1200 bytes");
    ByteBuffer out =
        ByteBuffer.allocate(HEADER_LENGTH + packet.payload.length + 4).order(ByteOrder.BIG_ENDIAN);
    ...
    out.put((byte) packet.copyIndex)
        .put((byte) packet.prn)
        .putShort((short) packet.week)
        .putShort((short) packet.intervalOfWeek)
        .put((byte) packet.timeOfInterval)
        .put((byte) 0)
        .putLong(packet.sentUtcTicks)
        .putShort((short) packet.payload.length)
        .putShort((short) 0)
        .put(packet.payload);
  ...
  public static Packet decode(byte[] bytes) {
    ...
    if (version != VERSION || headerLength != HEADER_LENGTH)
      throw new IllegalArgumentException("Unsupported AFS UDP header");
    ...
    if (in.getShort() != 0
        || payloadLength > MAXIMUM_PAYLOAD_LENGTH
        || bytes.length != HEADER_LENGTH + payloadLength + 4)
      throw new IllegalArgumentException("Invalid AFS UDP payload length");
    long expected =
        Integer.toUnsignedLong(
            ByteBuffer.wrap(bytes, bytes.length - 4, 4).order(ByteOrder.BIG_ENDIAN).getInt());
    if (Hashing.crc32(bytes, 0, bytes.length - 4) != expected)
      throw new IllegalArgumentException("AFS UDP CRC32 mismatch");
