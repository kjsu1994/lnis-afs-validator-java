...
class CodecTests {
  @Test
  void afsPacketRoundTrips() {
    var packet =
        new AfsPacketCodec.Packet(
            AfsPacketCodec.Kind.FRAME,
            UUID.randomUUID(),
            42,
            2,
            8,
  ...
  @Test
  void uuidUsesCSharpBigEndianGuidLayout() {
    UUID id = UUID.fromString("00112233-4455-6677-8899-aabbccddeeff");
    var packet =
        new AfsPacketCodec.Packet(AfsPacketCodec.Kind.PROBE, id, 0, 0, 8, 0, 0, 0, 0, new byte[0]);
  ...
  @Test
  void dropSimulationIsDeterministic() {
    assertEquals(
        AfsDropSimulator.shouldDrop(100, 2, 25, 7), AfsDropSimulator.shouldDrop(100, 2, 25, 7));
