...
class CombinedArtifactServiceTest {
  ...
  void createsCombinedJsonAndFourSheetWorkbook() throws Exception {
    ...
    when(sessions.result(sessionId, AgentRole.SENDER)).thenReturn(Optional.of(sender));
    when(sessions.result(sessionId, AgentRole.RECEIVER)).thenReturn(Optional.empty());
    when(evidence.details(sessionId)).thenReturn(List.of());
    CombinedArtifactService service = new CombinedArtifactService(sessions, evidence, json);
    ...
    try (XSSFWorkbook book = new XSSFWorkbook(new ByteArrayInputStream(xlsx))) {
      assertEquals(
          ...
          java.util.stream.IntStream.range(0, book.getNumberOfSheets())
              ...
              .toList());
      assertEquals("역할", book.getSheet("요약").getRow(0).getCell(0).getStringCellValue());
      assertEquals("DecodedFrames", book.getSheet("지표").getRow(1).getCell(2).getStringCellValue());
      assertEquals("SENDER", book.getSheet("시계열").getRow(1).getCell(0).getStringCellValue());
    }
  }

  private static RoleResult result(UUID sessionId) {
    IntegrityResult integrity = new IntegrityResult(true, 464, 464, "SOURCE", "SOURCE", 4, 4, "일치");
    Metric metric =
        new Metric(
    ...
    return new RoleResult(
        ...
        List.of(metric),
        counters,
        List.of(new ResourceSample(Instant.parse("2026-08-24T00:00:00Z"), 3.5, 1024)),
        null);
  }
}
