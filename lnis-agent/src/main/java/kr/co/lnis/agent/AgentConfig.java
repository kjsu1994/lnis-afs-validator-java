...
public record AgentConfig(String agentId, AgentRole role, URI serverUri, String token, Path nativeDirectory) {
    public static AgentConfig load(String[] args) {
        ...
        if (Files.isRegularFile(path)) try (var in = Files.newInputStream(path)) { file.load(in); }
        catch (Exception e) { throw new IllegalStateException("Unable to read " + path, e); }
