package kr.co.lnis.agent.nativecodec;
...
public final class NativeAfsCodec implements AutoCloseable {
    ...
    private void check(String operation, int code) {
        ...
        throw new IllegalStateException("AFS native " + operation + " failed (" + code + "): " + (pointer == null ? "unknown" : pointer.getString(0, "UTF-8")));
    }
    private static void require(int toi, byte[] sb2, byte[] sb3, byte[] sb4) {
        if (toi < 0 || toi > 99 || sb2.length != 1176 || sb3.length != 846 || sb4.length != 846) throw new IllegalArgumentException("Invalid AFS block length or TOI");
        for (byte[] block : List.of(sb2, sb3, sb4)) for (byte value : block) if (value != 0 && value != 1) throw new IllegalArgumentException("AFS inputs must be bits");
    }
