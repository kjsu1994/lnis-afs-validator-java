...
public final class Sb2PayloadCodec {
    ...
    public static byte[] encode(int week, int afsIntervalOfWeek, int prn) {
        ...
        if (afsIntervalOfWeek < 0 || afsIntervalOfWeek > 0x1ff) throw new IllegalArgumentException("AFS ITOW must be 0 to 511");
    ...
    public static Sb2EphemerisResult decode(
        ...
        boolean ephemerisMatches = equalRange(
                bits, expected, EPHEMERIS_OFFSET, EPHEMERIS_OFFSET + EPHEMERIS_BITS);
        ...
        return new Sb2EphemerisResult(
                configured.profileId(), prn, week, afsItow, toe * 16,
                ecc * POW2_M32, sqrta * POW2_M19,
                inc0 * POW2_M31 * Math.PI, omg0 * POW2_M31 * Math.PI,
                aop * POW2_M31 * Math.PI, m0 * POW2_M31 * Math.PI,
                toc * 16, af0 * POW2_M31, af1 * POW2_M43,
                week == packetWeek && afsItow == packetAfsIntervalOfWeek,
                ephemerisMatches, tailValid);
    }
    ...
    private static EphemerisProfile profile(int prn, double meanAnomalyRadians) {
        ...
        return new EphemerisProfile(
                prn, 319488.0, 0.6, 2557.342371, 0.9826203689,
                ascendingNodeRadians, 1.570796327, meanAnomalyRadians,
                319488.0, 0.0, 0.0);
    }
