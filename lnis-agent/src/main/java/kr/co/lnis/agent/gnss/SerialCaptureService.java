...
public final class SerialCaptureService implements AutoCloseable {
    ...
    public synchronized void start(Settings settings, Consumer<CaptureChunk> chunks, Consumer<Throwable> failure) {
        ...
        port.setNumStopBits(SerialPort.ONE_STOP_BIT); port.setParity(SerialPort.NO_PARITY); port.setComPortTimeouts(SerialPort.TIMEOUT_READ_SEMI_BLOCKING, 1000, 1000);
        ...
        try {
            if (!"raw-only".equalsIgnoreCase(settings.protocolId)) {
                byte[] metadata = GrawCodec.encode(new GrawCodec.Envelope(testId, UUID.randomUUID(), sequence++, Instant.now(),
                        new GrawCodec.ReceiverMetadata(settings.receiverModel, settings.firmwareVersion, settings.portName, settings.baudRate, settings.sessionName)));
                writeRecord(canonicalChunk, metadata); records++;
            }
            byte[] buffer = new byte[8192];
            while (running.get()) {
                int count = port.readBytes(buffer, buffer.length); if (count < 0) throw new IllegalStateException("Serial read failed"); if (count == 0) continue;
                ...
                if ("ubx".equalsIgnoreCase(settings.protocolId)) for (var frame : ubx.push(buffer, count)) {
                    var message = UbloxParser.toCanonical(frame); if (message != null) { writeRecord(canonicalChunk, GrawCodec.encode(new GrawCodec.Envelope(testId, UUID.randomUUID(), sequence++, Instant.now(), message))); records++; }
            ...
            if (rawChunk.size() > 0 || canonicalChunk.size() > 0) chunks.accept(new CaptureChunk(chunkIndex, drain(rawChunk), drain(canonicalChunk), bytesRead, records));
    ...
    private static void writeRecord(ByteArrayOutputStream out, byte[] record) { out.writeBytes(ByteBuffer.allocate(4).order(ByteOrder.BIG_ENDIAN).putInt(record.length).array()); out.writeBytes(record); }
    ...
    private void configureUbloxTemporarily() {
        ...
        for (int[] message : List.of(new int[]{0x02,0x15},new int[]{0x02,0x13})) {
            ...
            if(original!=null){restoreCommands.add(UbloxParser.command(0x06,0x01,original));byte[] enabled=original.clone();if(enabled.length>=8){enabled[3]=1;enabled[5]=1;}else if(enabled.length>=3)enabled[2]=1;write(UbloxParser.command(0x06,0x01,enabled));}
    ...
    private byte[] pollMessageRate(int messageClass,int messageId) {
        write(UbloxParser.command(0x06,0x01,new byte[]{(byte)messageClass,(byte)messageId}));UbloxParser parser=new UbloxParser();long deadline=System.nanoTime()+1_500_000_000L;byte[] buffer=new byte[2048];
        while(System.nanoTime()<deadline){int count=port.readBytes(buffer,buffer.length);if(count<=0)continue;for(var frame:parser.push(buffer,count))if(frame.messageClass()==0x06&&frame.messageId()==0x01&&frame.payload().length>=3&&(frame.payload()[0]&0xff)==messageClass&&(frame.payload()[1]&0xff)==messageId)return frame.payload();}return null;
    }
    ...
    private void write(byte[] value){if(port.writeBytes(value,value.length)!=value.length)throw new IllegalStateException("Unable to configure u-blox receiver");}
