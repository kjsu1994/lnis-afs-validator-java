...
import server.codec.NativeAfsCodec;
import server.config.AgentConfig;
import server.session.transport.UdpSessionService;
...
public final class AgentRuntime implements AutoCloseable {
    ...
    public void handle(Envelope envelope) {
        if (envelope.protocolVersion()
                != server.model.AgentProtocol.PROTOCOL_VERSION) {
            return;
        }
        try {
    ...
    private void startReceiver(UUID sessionId, JsonNode args) throws Exception {
        if (config.role() != server.model.LnisModels.AgentRole.RECEIVER) {
    ...
    private void startSender(UUID sessionId, JsonNode args) throws Exception {
        if (config.role() != server.model.LnisModels.AgentRole.SENDER) {
