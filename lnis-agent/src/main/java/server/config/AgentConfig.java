...
import static server.model.LnisModels.AgentRole;
...
public class AgentConfig {
  ...
  public static AgentConfig load(String[] args) {
    ...
    AgentRole role =
        AgentRole.valueOf(
            value("LNIS_AGENT_ROLE", "lnis.agent.role", file, "SENDER").toUpperCase(Locale.ROOT));
    URI uri =
        URI.create(
            value("LNIS_SERVER_WS", "lnis.server.ws", file, "ws://localhost:8088/lnis/agent/ws"));
  ...
  /** 역할별 실행 스크립트가 공백이 포함된 절대 경로도 안전하게 전달할 수 있도록 환경 변수 설정 파일 경로를 시스템 속성보다 우선한다. */
