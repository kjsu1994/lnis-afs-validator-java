package kr.co.lnis.server.input.repository;
...
@Repository
/** 입력 메타데이터와 바이너리 청크를 분리된 Redis 키로 저장한다. */
public class InputBufferRepository {
    private final StringRedisTemplate text;
    private final RedisTemplate<String, byte[]> binary;
    private final ObjectMapper json;

    public InputBufferRepository(
            StringRedisTemplate text,
            RedisTemplate<String, byte[]> binary,
            ObjectMapper json) {
        this.text = text;
        this.binary = binary;
        this.json = json;
    ...
    public void save(InputBufferEntity value, Duration ttl) {
        try {
            text.opsForValue().set(meta(value.inputId()), json.writeValueAsString(value), ttl);
        } catch (Exception e) {
    ...
    public Optional<InputBufferEntity> find(UUID id) {
        String value = text.opsForValue().get(meta(id));
        if (value == null) {
            return Optional.empty();
        }
        try {
            return Optional.of(json.readValue(value, InputBufferEntity.class));
        } catch (Exception e) {
            throw new IllegalStateException(e);
        }
    }
