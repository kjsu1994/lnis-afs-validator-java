package server.session.service;
...
import server.model.AgentProtocol.CommandType;
import server.model.LnisModels.*;
import server.agent.repository.AgentRepository;
import server.agent.service.AgentCommandService;
 ...
 *
 * <p>세션 생성 시 입력과 Agent 역할을 검증하고 H2 잠금 행을 획득한다. Receiver를 먼저 arm한 다음 Sender에 GRAW 청크를 전달하고 송신을
 * 시작한다. 모든 실패 경로에서는 가능한 한 lock을 해제해 다음 시험이 영구적으로 막히지 않게 한다.
 */
